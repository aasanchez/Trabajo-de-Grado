...
//Variables
int i;
void setGyro(s8 address,s8 val){
	i=I2C_write(0x68,address,2);
	I2C_STOP();
	i=I2C_write(0x68,val,2);
	I2C_STOP();
}

void setup() {
	init_I2C();									//Iniciamos el Protocolo i2c
	for (i=0;i<8;i++) i2c_buffer[i]=0;	//Limpiamos el buffer
	// Definimos el modo de trabajo del Giroscopo
	// Velocidad de Muestreo 1kHz, Filtro del Ancho de Banda 42Hz, Rango 500 d/s 
	setGyro(1,1);
}
